...
<%
    //Se recuperan los datos de un cliente, se modifican y se guardan en la base de datos
    AccesoBD abd =((AccesoBD)session.getAttribute("conex"));
    abd.sesionIniciada(session);
    try{
        abd.iniciarTransaccion();
        Long idClie = Long.parseLong(request.getParameter("id"));
        ...
        Cliente cli = (Cliente) abd.buscarPorId(new Cliente(), idClie);
        ...
        f.setMonth(mes-1);
        f.setYear(anio-1900);
        m.ModCliente(cli, email, nombre, apellido, dni, direccion, telFijo, telMovil, localidad, f);
